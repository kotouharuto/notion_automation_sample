...
def format_notion_results(data):
    ...
    for r in results:
       ...
       # 日にち関連列
        start_date = props.get("対応開始日", {}).get("date", {}).get("start", "No Start Date")
        end_date = props.get("対応終了予定日", {}).get("date", {}).get("start", "No End Date")

        # Notes列（rich_text）も通知に含めたい場合
        notes_prop = props.get("Notes", {}).get("rich_text", [])
        notes_text = notes_prop[0]["plain_text"] if notes_prop else ""

        # Slack用の1行メッセージ作成
        line = f"- {name_text} ({status}),  期間: ({start_date} ~ {end_date})"
        if notes_text:
            line += f" : {notes_text}"
        tasks.append(line)
